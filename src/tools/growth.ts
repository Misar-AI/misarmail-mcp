...
export const growthTools: ToolDefinition[] = [
  ...
  defineTool({
    ...
    category: "forms",
    description:
      "Create a hosted landing page with a single hero block (headline, subheadline, CTA). Returns the public URL. The real API stores pages as a page-builder block array with no dedicated headline/CTA fields, so this tool translates your inputs into one 'hero' block — for multi-block pages or a real CTA link, use the landing page editor in the MisarMail dashboard.",
    scopes: ["write"],
    annotations: {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        slug: { type: "string", description: "URL slug (auto-generated from name when omitted)" },
      },
    },
    handler: (ctx, args) =>
      apiFetch(ctx, "/landing-pages", {
        method: "POST",
        body: JSON.stringify({
          title: args.name,
          slug: args.slug,
          blocks: [
            {
              ...
              content: {
                ...
                subheading: args.subheadline,
                cta_label: args.cta_text,
                cta_url: "#",
                align: "center",
              },
            },
